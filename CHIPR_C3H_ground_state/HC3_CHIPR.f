C###############################################################################
C HC3 GROUND-STATE POTENTIAL IN CARTESIAN COORDINATES (A.U)
C
C C. M. R. ROCHA AND A. J. C. VARANDAS (TO BE SUBMITTED) 2019
C
...
C X(7) - X(9)  : X, Y, Z FOR C3
C X(10)- X(12) : X, Y, Z FOR C4
C
C POT IS THE POTENTIAL IN HARTREES
C
C###############################################################################

      SUBROUTINE FCNHC3(N,X,POT)
      IMPLICIT NONE
      ...
      DO I=1,INTDIST
        W(I)=R(I)
      END DO
      CALL POTHC3(W,POT,DER,DVDR)
      RETURN
      END

C###############################################################################
C HC3 GROUND-STATE POTENTIAL IN CARTESIAN COORDINATES (A.U) AND FIRST DERIVATIVES
C
C C. M. R. ROCHA AND A. J. C. VARANDAS (TO BE SUBMITTED) 2019
C
C N IS 3X THE NUMBER OF ATOMS, IN THIS CASE 12
...
C###############################################################################
C HC3 GROUND-STATE DOUBLET POTENTIAL ENERGY SURFACE IN A.U
C
C C. M. R. ROCHA AND A. J. C. VARANDAS (TO BE SUBMITTED) 2019
C
...
C
C SINGLET C3 POTENTIAL FROM ROCHA AND VARANDAS, CHEM. PHYS. LETT. 700, 36-43 (2018)
...
C
C FOUR-BODY TERMS USING CHIPR AND CALIBRATED FROM 42538 CCSD(T)/CBS(d,t) ENERGIES
C
C R1, R2, AND R3 DEFINE C-H BOND DISTANCES
C R4, R5, AND R6 DEFINE C-C BOND DISTANCES
C
...
C##############################################################################
      ...
      SUBROUTINE POTHC3(R,POT,DER,DVDR)
      IMPLICIT NONE
      INTEGER :: I
      ...
      CALL POT23HC3(R,POT23)
      CALL CHIPR_TETRA_HC3(R,POT4,DER,DVDR4BD)
...
C###############################################################################
C SUM OF TWO- PLUS THREE-BODY TERMS FOR GROUND-STATE DOUBLET HC3
C###############################################################################      
      
      SUBROUTINE POT23HC3(R,POT)
      IMPLICIT NONE
      INTEGER :: I
      INTEGER, PARAMETER :: NATOM=4
      ...
      DOUBLE PRECISION :: POT, POT4BDHC3
...
C THREE-BODY TERMS FOR C3 TRIPLET GROUND-STATE
          CALL CHIPR_TRIAT_T_C3(WIPT,POT3BD2(I))          
        ELSE 
...
C          STOP
C        ELSE
C           CALL CHIPR_TETRA_HC3(WIPQ,POT4BD(I),DER,DVDR3BD)
C           POT4BDHC3=POT4BDHC3+POT4BD(I)
C        END IF 
C      END DO
...
C###############################################################################
      ...
      CALL DIAG23(V,EIGVAL,VEC,2,RHO)
...
C######################################################################################################
C RANG FUNCTION FOR TWO PLUS THREE TERMS
C######################################################################################################

      DOUBLE PRECISION FUNCTION RANG23BD(R,X0,GAM)
      IMPLICIT NONE
      DOUBLE PRECISION :: R,X0,GAM
      RANG23BD=0.50D+00*(1.00D+00+TANH(GAM*(R-X0)))
      END FUNCTION RANG23BD      

C###############################################################################
C FOURD-BODY TERM FOR GROUND-STATE DOUBLET HC3
C###############################################################################

      SUBROUTINE CHIPR_TETRA_HC3(R,POT,DER,DVDR)
      IMPLICIT NONE
      CHARACTER(LEN=4), PARAMETER :: MOLTYP="AB3"
      INTEGER, PARAMETER :: DEG=2
      INTEGER, PARAMETER :: NC=376
      ...
      DOUBLE PRECISION :: MARK,A1,B1,C1,D1,F1,F2      
      C( 369)= -0.23621337498570156576693790384524618D-01
      C( 370)=  0.14879628246743806241347840568778338D+00
      C( 371)= -0.20401580130012289373553358018398285D+04
      C( 372)=  0.36693016363271775404086838534567505D+01
      C( 373)=  0.25306835800533087343922034051502123D+01
      C( 374)=  0.24437977187557184954869171633617952D+01
      C( 375)=  0.11099601108105230373723770753713325D+01
      C( 376)=  0.23664752705757163830924127978505567D+01
      ...
      DO I=1,DEG
        ...
        IF (DEG.EQ.2) THEN
          IF (I.EQ.1) THEN
            DO O=1,3
               CALL BASIS_CNTRA(1,BSORDER(1),BS,F1,F2,A1,B1,R(O),Y(O))
            IF (DER) THEN
            CALL DBASIS_CNTRA(1,BSORDER(1),BS,F1,F2,A1,B1,R(O),DYDR(O))
            END IF               
            END DO            
          ELSE
            DO O=4,6 
               CALL BASIS_CNTRA(2,BSORDER(2),BS,F1,F2,C1,D1,R(O),Y(O))
            IF (DER) THEN               
            CALL DBASIS_CNTRA(2,BSORDER(2),BS,F1,F2,C1,D1,R(O),DYDR(O))
            END IF
            END DO
          END IF            
        END IF
...
C
     ...
     *          .AND. S.NE.L .AND. S.NE.N .AND. S.NE.O) THEN
                ...
                IF (S.NE.(I+J+K) .AND. S.NE.(I+J+L) .AND.  
     *          S.NE.(I+J+N) .AND.             
     ...
     *               Y(4)**(P(4,LL))*Y(5)**(P(5,LL))*Y(6)**(P(6,LL)))
                   ...
                   ELSE IF (MOLTYP.EQ."AB3") THEN
                     ...
                     IF (I.LE.J .AND. L.LE.N .AND. 
     *                   N.LE.O .AND. J.LE.K) THEN
...
C################################################################################
C DAMPING THE 4-BODY ENERGY TERM AT SHORT INTERNUCLEAR DISTANCES
C################################################################################ 
      ...
      XI=1.0D+00
      DO I=1,NX
        H(I)=(0.5D+00*(1.00D+00+TANH(KAPPA*(R(I)-R0))))**(XI) 
      END DO
      REPDAMPTETRA=(PRODUCT(H))   
      END FUNCTION
      
!################################################################################
! DAMPING THE 4-BODY ENERGY TERM AT LONG INTERNUCLEAR DISTANCES
!################################################################################ 

      DOUBLE PRECISION FUNCTION LRDAMPTETRA(NX,R)
      INTEGER :: I
      DOUBLE PRECISION, DIMENSION(NX) :: R,H
      DOUBLE PRECISION :: KAPPA, XI, R0
      R0=10.0D+00
...
C###############################################################################
      ...
      SUBROUTINE DIAG23(A,EIG,VEC,NN,RHO)
      PARAMETER(NDIM=2)
...
C LEE ROY RADIUS (R0) OF C-Mg
      ...
      CALL TRIANG2JACOBIC3(R1,R2,R3,RE,rjac,cosgamma,RB,RC)
...
C####################################################################################
      ...
      SUBROUTINE TRIANG2JACOBIC3(R1,R2,R3,RE,RJAC,COSGAMMA,RB,RC)
      IMPLICIT NONE
      ...
      END SUBROUTINE TRIANG2JACOBIC3
...
C###############################################################################
      ...
      SUBROUTINE CHIPR_TRIAT_T_C3(R,POT)
      IMPLICIT NONE
...
C######################################################################################################
C C2 SIGMA G PLUS SINGLET EXPERIMENTALLY-DETERMINED POTENTIAL ENERGY CURVE
C######################################################################################################
...
C######################################################################################################
C C2 PI U TRIPLET EXPERIMENTALLY-DETERMINED POTENTIAL ENERGY CURVE
C######################################################################################################
...
C################################################################################
C FINDING THE POSSIBLE PERMUTATIONS OF I J K R S T FOR AN ABCD-TYPE MOLECULE
C################################################################################      
...
C     ACTION OF THE INDENTITY OPERATOR ONLY
...
C################################################################################
C FINDING THE POSSIBLE PERMUTATIONS OF I J K R S T FOR AN AB3-TYPE MOLECULE
C Y1, Y2, AND Y3 IS ALWAYS THE A-B BONDS
C Y4, Y5, AND Y6 CORRESPONDS TO THE B-B BONDS
C################################################################################      
      ...
      INTEGER, DIMENSION(6) :: INTER
      INTEGER, DIMENSION(6,6) :: P,PINT,PRED      
      ...
      END DO
C
C     GENERATING ALL POSSIBLE PERMUTATIONS
C     THE REDUNDANT ONES ARE LATTER EXCLUDED
C
      INTER(1)=I
      INTER(2)=J
      INTER(3)=K
...
C     ACTION OF THE INDENTITY OPERATOR
...
C     ACTION OF THE (2,3,4) OPERATOR
...
C     ACTION OF THE (2,4,3) OPERATOR
      ...
      END DO
C
C     EXCLUDING REDUNDANT EXCITATIONS
C
C
C     FIRST, MARK THEN WITH THE NUMBER 1000
C
      DO L=1,6
        DO M=(L+1),6
          SS=0
      ...
      END DO
C
C     SECOND, EXCLUDE THOSE MARKED FROM THE VECTOR
C      
      DO L=1,6 
        V=0
        DO M=1,6
      ...
      END DO      
C
C     DEFINE A THE NEW PERMUTATION OPERATORS
C
      DO L=1,6 
        DO M=1,6
          P(L,M)=0
...
C################################################################################
C SUBROUTINE TO CALCULATE THE NUMBER OF COEFFICIENTS NC FOR A GIVEN POLYNOMIAL
C OF ORDER "ORDER" AND MOLECULE TYPE "MOLTYP"
C################################################################################       
     ...
     *          S.NE.(K+N+O) .AND. S.NE.(J+N+O)) THEN
C
C     FOR THE CASE OF AN ABCD MOLECULE 
C      
                   IF (MOLTYP.EQ."ABCD") THEN  
                     ...
                     MNUM=MNUM+1                      
C
C     FOR THE CASE OF AN AB3 MOLECULE 
C
                   ELSE IF (MOLTYP.EQ."AB3") THEN

                     IF (I.LE.J .AND. L.LE.N 
     *               .AND. N.LE.O .AND. J.LE.K) THEN
                     ...
                     END IF
C
C     FOR THE CASE OF AN A2B2 MOLECULE 
C
                   ELSE IF (MOLTYP.EQ."A2B2") THEN

                     STOP
C
C     FOR THE CASE OF AN ABC2 MOLECULE 
C
                   ELSE IF (MOLTYP.EQ."ABC2") THEN

                     STOP
C
C     FOR THE CASE OF AN A4 MOLECULE 
C
                   ELSE IF (MOLTYP.EQ."A4") THEN
...
C######################################################################################################
      ...
      SUBROUTINE BASIS_CNTRA(DEG,M,C,F1,F2,P1,P2,R,YVAL)
      IMPLICIT NONE
      INTEGER :: I,J,DEG
      INTEGER :: M
      DOUBLE PRECISION :: RREF0,ZETA
...
C######################################################################################################
      ...
      SUBROUTINE DBASIS_CNTRA(DEG,M,C,F1,F2,P1,P2,R,DYDR)
      IMPLICIT NONE
      INTEGER :: I,J,DEG
      INTEGER :: M
      DOUBLE PRECISION :: RREF0,ZETA
      ...
      DOUBLE PRECISION :: P1,P2,SECH
      DOUBLE PRECISION :: PART1,PART2
      DOUBLE PRECISION :: PART3,PART4
      PART1=F1
      PART2=P1
      PART3=(SECH(P1*(R-P2)))**2
      DYDR=-PART1*PART2*PART3
      RETURN
      END
