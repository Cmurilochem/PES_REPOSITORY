...
C N is 3x the number of atomns
C X the initial cartezian coordinates
C POT the potentials
      IMPLICIT NONE
      INTEGER, PARAMETER :: NATOM=4
      INTEGER, PARAMETER :: INTDIST=(NATOM*(NATOM-1))/2
      INTEGER, PARAMETER :: NATOMMAX=100
      DOUBLE PRECISION, DIMENSION(INTDIST) :: W, Z
      INTEGER :: I, N
...
C###############################################################################
      ...
      SUBROUTINE SIMANN(N,X,XOPT,FOPT,IER)
C  N is the dimension of the problem
C  X is the vector of initial cartezian coordinates
C  XOPT optima coordinates
C  FOPT minimum value
C
...
C  Set input values of the input/output parameters.
 ...
 20   CONTINUE
  ...
  50  FORMAT ("****************************************************")
1000  FORMAT(/,'4). STARTING SIMULATED ANNEALING',/,
     1       /,"****************************************************",/,
     2       /,' NUMBER OF PARAMETERS: ',I3,'   MAXIMAZATION: ',L5,
     3       /,' INITIAL TEMP: ', G8.2, '   RT: ',G8.2, '   EPS: ',G8.2,
     4       /,' NS: ',I3, '   NT: ',I2, '   NEPS: ',I2,
     5       /,' MAXEVL: ',I10, '   IPRINT: ',I1, '   ISEED1: ',I4,
     6       '   ISEED2: ',I4)
